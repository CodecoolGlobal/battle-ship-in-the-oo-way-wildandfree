            ...
            for (int i = 0; i < grid.GetLength(0); i++)
            {
                for (int j = 0; j < grid.GetLength(1); j++)
                {
                    ...
                    else if (j == 0 || j == grid.GetLength(1))
                    {
                        ...
                        grid[i, j] = (new Cell(i, j, letter)); // to jest frame - cyfry!
                    }
                    else if (i == 1 || i == grid.GetLength(0) - 1)
                    {
                        string horizontal = "|";
                        grid[i, j] = (new Cell(i, j, horizontal)); // to jest frame kreski pionowe!
                    }
                    else if (j == 1 || j == grid.GetLength(1) - 1)
                    {
                        string vertical = "–";
                        grid[i, j] = (new Cell(i, j, vertical)); // to jest frame kreski poziome!
                    }
                    else if (i == 2 || i == grid.GetLength(0) - 2 || j == 2 || j == grid.GetLength(1) - 2)
                    {
                        grid[i, j] = (new Cell(i, j, true)); // nie można postawić statku
                    }
                    else
                    {
                        grid[i, j] = (new Cell(i, j, false)); // można postawić statek 
                    }
                }
